...
const PreferencesProvider = ({ children }) => {
    ...
    const getPreference = (0, react_1.useCallback)(async (key) => {
        ...
        const promise = new Promise((resolve) => {
            (async () => {
                const request = await api_1.requests.get(`${serverURL}${api}/payload-preferences/${key}`, {
                    headers: {
    ...
    const setPreference = (0, react_1.useCallback)(async (key, value) => {
        preferencesRef.current[key] = value;
        await api_1.requests.post(`${serverURL}${api}/payload-preferences/${key}`, requestOptions(value, i18n.language));
