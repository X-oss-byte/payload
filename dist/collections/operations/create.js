...
async function create(incomingArgs) {
    ...
    const { collection, collection: { config: collectionConfig, }, req, req: { payload, payload: { config, emailOptions, }, }, disableVerificationEmail, depth, overrideAccess, showHiddenFields, overwriteExistingFiles = false, draft = false, autosave = false, } = args;
    try {
        ...
        let result = (0, sanitizeInternalFields_1.default)(doc);
        // /////////////////////////////////////
        // Create version
        // /////////////////////////////////////
        ...
        // Remove temp files if enabled, as express-fileupload does not do this automatically
        if (((_b = config.upload) === null || _b === void 0 ? void 0 : _b.useTempFiles) && collectionConfig.upload) {
            const { files } = req;
            const fileArray = Array.isArray(files) ? files : [files];
            await (0, mapAsync_1.mapAsync)(fileArray, async ({ file }) => {
                // Still need this check because this will not be populated if using local API
