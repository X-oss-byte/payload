    ...
    ctx.config.collections = ctx.config.collections.map((collection) => {
        ...
        if (collection.auth) {
            ...
            if (collection.auth.useAPIKey) {
                passport_1.default.use(`${config.slug}-api-key`, (0, apiKey_1.default)(ctx, config));
            }
            if (Array.isArray(collection.auth.strategies)) {
                collection.auth.strategies.forEach(({ name, strategy }, index) => {
                    ...
                    passport_1.default.use(`${config.slug}-${name !== null && name !== void 0 ? name : index}`, passportStrategy);
                });
            }
        }
